...
def _list_LTTextBoxHorizontal_2_matrix(
    list_LTTextBoxHorizontal: list[LTTextBoxHorizontal],
    ...
    list_LTTextBoxHorizontal = sorted(
        list_LTTextBoxHorizontal, key=lambda box: box.y0, reverse=True
    )

    # Initialising matrix with the 1st most top element on the page
    matrix = [[list_LTTextBoxHorizontal[0]]]
    ...
    for i in range(1, len(list_LTTextBoxHorizontal)):
        vert_middle_prev_box = (
            list_LTTextBoxHorizontal[i - 1].y0 + list_LTTextBoxHorizontal[i - 1].y1
        ) / 2
...
def _PDFpage2txt(page: PDFPage, laparams=None) -> str:
    ...
    matrix_of_LTTextBoxHorizontal = _list_LTTextBoxHorizontal_2_matrix(
        list_LTTextBoxHorizontal
    )
...
def pdf_2_txt_file(
    pdf_file_name: str,
    txt_output_file_name: None | str = None,
    password="",
    page_numbers=None,
    maxpages=0,
    caching=True,
    ...
    pdf_text = pdf_2_text(
        pdf_file_name, password, page_numbers, maxpages, caching, laparams
    )
